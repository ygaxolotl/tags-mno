...
def convert_log_dict_to_np_lorenz96(logs):
        ...
        logs n_samples*[
            dict(
                ...
                'u': ((np.array(n_tgrid, K),: Low-res solution
                    ...
                    (np.array(n_tgrid, K))): Parametrization of middle-res onto low-res effects, y_param
                ): Solution
            )
        ]: n samples of the model
    ...
    n_samples_after_warmup = len(logs)
    stoch_dim = logs[0]['rand_insts'].shape[-1]
    dtype = logs[0]['u'][0].dtype

    # Initialize output vectors
    u_x = np.zeros((n_samples_after_warmup,) + logs[0]['u'][0].shape, dtype=dtype)
    u_y = np.zeros((n_samples_after_warmup,) + logs[0]['u'][1].shape, dtype=dtype)
    u_z = np.zeros((n_samples_after_warmup,) + logs[0]['u'][2].shape, dtype=dtype)
    y_param = np.zeros((n_samples_after_warmup,) + logs[0]['u'][3].shape, dtype=dtype)
    
    u_x0 = np.zeros((n_samples_after_warmup,) + logs[0]['rand_ic'][0].shape, dtype=dtype)
    u_y0 = np.zeros((n_samples_after_warmup,) + logs[0]['rand_ic'][1].shape, dtype=dtype)
    u_z0 = np.zeros((n_samples_after_warmup,) + logs[0]['rand_ic'][2].shape, dtype=dtype)
    
    rand_param = np.zeros((n_samples_after_warmup, logs[0]['rand_param'].shape[0]), dtype=dtype)
    rand_insts = np.zeros((n_samples_after_warmup, stoch_dim), dtype=dtype)

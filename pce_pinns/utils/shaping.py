...
def flatten_to_0D_data(grid, y, y_args=None, rand_insts=None, 
    ...
    Args:
        ...
        test_mode str: see interpolate_param_nn
    Returns:
    ...
    if batch_size is None:
        if mode == 'sequential_coupled_local':
            batch_size = int(np.prod(y_args.shape[1:3]))
        else:
...
def shape_2D_to_2D_nn_input(grid, y, y_args=None, rand_insts=None,
    ...
    Args:
        ...
        target_name string: Indication which target
        alpha_indices np.array((n_alpha_indices, dim_rand)): Set of alpha indices. If not None, used as prediction target and in PceLoss. See rom.pce.multi_indices()
        grid_in_dims tuple(int): Indices of grid dimensions that are used as input, e.g., () for none, -1 for all, 0, 1 for tgrid and x1grid.
    ...
    if batch_size is None or batch_size == 'None':
...
def split_sequences_into_snippets(sequences, n_snippets, n_tsnippet, verify_dims=True):
    """
    Splits long sequences of length,n_tgrid, into snippets of length, n_tsnippet.

    Args:
        sequences np.array(n_sequences, n_tgrid, ...)
        n_snippets int: Desired number of total snippets. Only full-length snippets will be created. The rest of the data will be discarded. .
        n_tsnippet int: Desired length of each snippet in time   
    Returns:
        snippets np.array(n_snippets, n_tsnippet, ...)
    """
    n_sequences = sequences.shape[0]
    n_tgrid = sequences.shape[1]
    n_snippets_per_sequence = math.floor(n_tgrid/n_tsnippet)   
    sequences = sequences[:,:(n_tsnippet*n_snippets_per_sequence)] # Truncate sequences to equal division by snippet length
    
    snippets = sequences[:,None,...] # Expand dimensions
    dims = sequences.shape[2:]
    # Split each sequence into snippets 
    snippets = snippets.reshape((n_sequences, n_snippets_per_sequence, n_tsnippet,) + dims) 
    # Concatenate snippets across first dimension
    snippets = snippets.reshape((n_sequences*n_snippets_per_sequence, n_tsnippet,) + dims)
    snippets = snippets[:n_snippets] # Truncate sequences to equal division by number of task sor cpus
    return snippets

...
def rdft1(x:torch.Tensor, m:Tuple[torch.Tensor]) -> torch.Tensor:
    """DFT transform for real-valued 1D input vectors.

    See :function:rdft2

    :param x: Input vector as ``(*, x)`` tensor.
    :param m: DFT matrix for dimension ``x`` 
    :return: Complex DFT transform of input as ``(*, fx, 2)`` tensor where the last dimension
             wraps the real and imaginary part, respectively.
    """
    x = torch.matmul(m, x.type(torch.complex64))
    return x
